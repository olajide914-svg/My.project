...
def load_and_prepare_data(file_path):
    ...
    try:
        ...
        print("Please make sure your CSV file is in the exact same folder as the .py script.")
...
def calculate_anomalies(df, ref_start=1991, ref_end=2020):
    ...
    monthly_data = df.groupby(['year', 'month'])[['tmax', 'tmin', 'tmean']].mean().reset_index()
    ...
    yearly_summer_anomalies = summer_anomalies.groupby('year')[
        ['tmax_anom', 'tmin_anom', 'tmean_anom']
    ].mean().reset_index()
...
def plot_summer_anomalies(anomaly_data):
    ...
    fig.suptitle('Summer (JJA) Temperature Anomalies for Graz (1922–2025)\nReference Period: 1991–2020', fontsize=16)
    ...
    for i, (param_col, title, orig_col) in enumerate(params):
        ...
        ax.bar(top_5['year'], top_5[param_col], color='#ff7f0e', edgecolor='black', linewidth=1, label='Top 5 Hottest')
        
        # Add labels to top 5
        for _, row in top_5.iterrows():
            ax.text(row['year'], row[param_col] + 0.1 * (1 if row[param_col] > 0 else -1), 
                    f"{int(row['year'])}", ha='center', va='bottom' if row[param_col] > 0 else 'top', fontsize=9)
...
def plot_monthly_statistics(df, birth_year):
    ...
    monthly_stats = df.groupby('month')[['tmax', 'tmin', 'tmean']].quantile(quantiles).unstack()

    # Get monthly data for the specific years to overlay
    monthly_data_plot = df.groupby(['year', 'month'])[['tmax', 'tmin', 'tmean']].mean().reset_index()
    yearly_data_to_plot = monthly_data_plot[monthly_data_plot['year'].isin(years_to_plot)]

    fig, axes = plt.subplots(3, 1, figsize=(15, 20), sharex=True)
    fig.suptitle('Monthly Temperature Statistics for Graz (Full Period: 1922–2025)', fontsize=16)
    ...
    if len(years_to_plot) > 1:
        colors = plt.cm.jet([i / (len(years_to_plot) - 1) for i in range(len(years_to_plot))])
    ...
    for i, (param, title, _) in enumerate(params):
        ...
        # Plot IDR (10th-90th)
        ax.fill_between(months, stats[0.1], stats[0.9], color='#cccccc', alpha=0.7, label='10th–90th Percentile (IDR)')
        
        # Plot IQR (25th-75th)
        ax.fill_between(months, stats[0.25], stats[0.75], color='#a0a0a0', alpha=0.9, label='25th–75th Percentile (IQR)')
        
        # Plot Median
        ax.plot(months, stats[0.5], color='black', linestyle='--', linewidth=2, label='Median (All Years)')
        ...
        for j, year in enumerate(years_to_plot):
            ...
            if not year_data.empty:
                ...
                ax.plot(year_data['month'], year_data[param], marker='o', markersize=5, 
                        linestyle='-', label=f'Year {year}', color=colors[j], linewidth=2)
...
def plot_extreme_heat_days(df):
    ...
    # Sum per year
    yearly_extremes = df.groupby('year')[['hot_day', 'tropical_night']].sum().reset_index()
    ...
    # Plot Hot Days
    axes[0].bar(yearly_extremes['year'], yearly_extremes['hot_day'], color='#d62728', label='Hot Days')
    axes[0].set_title('Yearly Number of Hot Days (Tmax ≥ 30°C)')
    axes[0].set_ylabel('Number of Days')
    axes[0].grid(axis='y', linestyle='--', alpha=0.7)
    
    # 10-year running mean for hot days
    yearly_extremes['hot_day_10yr_mean'] = yearly_extremes['hot_day'].rolling(window=10, center=True, min_periods=1).mean()
    axes[0].plot(yearly_extremes['year'], yearly_extremes['hot_day_10yr_mean'], color='black', linewidth=2, label='10-yr running mean')
    axes[0].legend()
    
    # Plot Tropical Nights
    axes[1].bar(yearly_extremes['year'], yearly_extremes['tropical_night'], color='#1f77b4', label='Tropical Nights')
    axes[1].set_title('Yearly Number of Tropical Nights (Tmin ≥ 20°C)')
    axes[1].set_ylabel('Number of Days')
    axes[1].grid(axis='y', linestyle='--', alpha=0.7)

    # 10-year running mean for tropical nights
    yearly_extremes['tropical_night_10yr_mean'] = yearly_extremes['tropical_night'].rolling(window=10, center=True, min_periods=1).mean()
    axes[1].plot(yearly_extremes['year'], yearly_extremes['tropical_night_10yr_mean'], color='black', linewidth=2, label='10-yr running mean')
    axes[1].legend()
...
def main():
    ...
    DATA_FILE_PATH = 'Messstationen_Graz_Tagesdaten_v2_Datensatz_19220101_20251031 (1).csv'
    ...
    if df_graz is not None:
        ...
        summer_anomalies_df = calculate_anomalies(df_graz, REF_PERIOD_START, REF_PERIOD_END)
